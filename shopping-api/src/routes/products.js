...
router.put('/reorder', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'Error reordering products',
      detail: error.message
    });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!targetStoreId) {
      ...
      if (!store) {
        return res.status(400).json({ error: 'No stores available' });
      }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    for (const key of PRODUCT_FIELDS) {
      ...
      if (
        ['heroImageUrl', 'image2Url', 'image3Url', 'image4Url', 'videoUrl'].includes(key) &&
        raw[key] === null
      ) {
        updateData[key] = null;
      }
    ...
    if (updateData.subPrice !== undefined) {
      updateData.subPrice = parseInt(updateData.subPrice, 10) || null;
    }
